...
import { currentTrack } from '../../setup/fixtures';
...
describe('WebPlayer', () => {
    ...
    const renderPlayer = (overrideProps = {}) => render(
        <WebPlayer {...activeProps} {...overrideProps} />
    );
    ...
    test('shows a Pause button when playing and a Play button when paused', () => {
        const { rerender } = renderPlayer({ is_paused: false });
    ...
    test('clicking Play or Pause calls player.togglePlay', () => {
        renderPlayer({ is_paused: false });
    ...
    test('repeat button shows the correct aria-label for each repeat mode', () => {
        const { rerender } = renderPlayer({ repeatMode: 0 });
    ...
    test('volume slider renders with the correct value and calls onVolumeChange on change', () => {
        ...
        renderPlayer({ volume: 0.75, onVolumeChange });
    ...
    test('mute button calls onMuteToggle when clicked', () => {
        ...
        renderPlayer({ volume: 0.6, onMuteToggle });
    ...
    test('unmute button calls onMuteToggle when volume is 0', () => {
        const onMuteToggle = vi.fn();
        renderPlayer({ volume: 0, onMuteToggle });
    ...
    test('displays the formatted position and duration times', () => {
        renderPlayer({ position: 30000, duration: 210000 });
    ...
    test('seek slider calls onSeek with the correct numeric value on pointer up', () => {
        ...
        fireEvent.pointerUp(screen.getByRole('slider', { name: 'Song position' }), {
            target: { value: '60000' },
        });
